...
\@x{}\moduleLeftDash\@xx{ {\MODULE} BitcoinChain}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Naturals ,\, FiniteSets ,\, Sequences}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANTS}}%
\@x{\@s{16.4} Nodes ,\,\@s{23.54}}%
\@y{\@s{0}%
 Set of nodes in the network
}%
\@xx{}%
\@x{\@s{16.4} MaxBlocks ,\,\@s{3.18}}%
\@y{\@s{0}%
 Maximum number of blocks that can be created. We need this to cap the model
 run.
}%
\@xx{}%
\@x{\@s{16.4} GenesisHash}%
\@y{\@s{0}%
...
\@pvspace{8.0pt}%
\@x{ {\VARIABLES}}%
\@x{\@s{16.4} blocksByNode ,\,\@s{16.99}}%
\@y{\@s{0}%
 \ensuremath{blocksByNode[n]} is the set of blocks known by node \ensuremath{n
}}%
\@xx{}%
\@x{\@s{16.4} chainsByNode ,\,\@s{14.43}}%
\@y{\@s{0}%
 \ensuremath{chainsByNode[n]} is the \ensuremath{DAG} of blocks for node
 \ensuremath{n
}}%
\@xx{}%
\@x{\@s{16.4} tipsByNode ,\,\@s{26.44}}%
\@y{\@s{0}%
 \ensuremath{tipsByNode[n]} is the current tip of the chain for node
 \ensuremath{n
}}%
\@xx{}%
\@x{\@s{16.4} workByNode ,\,\@s{21.46}}%
\@y{\@s{0}%
 \ensuremath{workByNode[n]} is a function mapping each block to its
 cumulative work
}%
\@xx{}%
\@x{\@s{16.4} confirmedByNode ,\,}%
\@y{\@s{0}%
 \ensuremath{confirmedByNode[n]} is the set of confirmed blocks for node
 \ensuremath{n
}}%
\@xx{}%
\@x{\@s{16.4} network ,\,\@s{41.23}}%
\@y{\@s{0}%
 \ensuremath{network[n]} is the set of blocks in transit to node \ensuremath{n
}}%
\@xx{}%
\@x{\@s{16.4} nextBlockId\@s{30.96}}%
\@y{\@s{0}%
 Counter used to generate unique block \ensuremath{IDs
}}%
\@xx{}%
\@pvspace{8.0pt}%
 \@x{ vars \.{\defeq} {\langle} blocksByNode ,\, chainsByNode ,\, tipsByNode
 ,\, workByNode ,\, confirmedByNode ,\, network ,\, nextBlockId {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Type definitions
...
\@xx{}%
 \@x{ Block\@s{2.36} \.{\defeq} [ id \.{:} Nat ,\, prevHash \.{:} Nat ,\,
 nonce \.{:} Nat ,\, timestamp \.{:} Nat ]}%
 \@x{ Chain \.{\defeq} [ blocks \.{:} {\SUBSET} Nat ,\, edges \.{:} {\SUBSET}
 ( Nat \.{\times} Nat ) ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ WorkFor ( b ) \.{\defeq} 1\@s{4.1}}%
\@y{\@s{0}%
 Simplified work calculation, each block contributes 1 unit of work
}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{}%
...
\@xx{}%
\@x{ Init \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} blocksByNode\@s{2.55} \.{=} [ n \.{\in} Nodes
 \.{\mapsto} \{ [ id \.{\mapsto} 0 ,\, prevHash \.{\mapsto} GenesisHash ,\,
 nonce \.{\mapsto} 0 ,\, timestamp \.{\mapsto} 0 ] \} ]}%
 \@x{\@s{16.4} \.{\land} chainsByNode \.{=} [ n \.{\in} Nodes \.{\mapsto} [
 blocks \.{\mapsto} \{ 0 \} ,\, edges \.{\mapsto} \{ \} ] ]}%
 \@x{\@s{16.4} \.{\land} tipsByNode\@s{4.98} \.{=} [ n \.{\in} Nodes
 \.{\mapsto} 0 ]}%
 \@x{\@s{16.4} \.{\land} workByNode \.{=} [ n \.{\in} Nodes \.{\mapsto} [ i
 \.{\in} \{ 0 \} \.{\mapsto} 1 ] ]\@s{4.1}}%
\@y{\@s{0}%
 Initialize work for genesis block to 1
}%
\@xx{}%
 \@x{\@s{16.4} \.{\land} confirmedByNode\@s{1.03} \.{=} [ n \.{\in} Nodes
 \.{\mapsto} \{ \} ]}%
\@x{\@s{16.4} \.{\land} network \.{=} [ n \.{\in} Nodes \.{\mapsto} \{ \} ]}%
\@x{\@s{16.4} \.{\land} nextBlockId \.{=} 1}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ CreateBlock ( n ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} nextBlockId \.{<} MaxBlocks}%
\@x{\@s{16.4} \.{\land} \.{\LET}}%
\@x{\@s{43.91} newBlock \.{\defeq} [ id \.{\mapsto} nextBlockId ,\,}%
\@x{\@s{104.13} prevHash \.{\mapsto} tipsByNode [ n ] ,\,}%
\@x{\@s{104.13} nonce \.{\mapsto} nextBlockId ,\,\@s{4.1}}%
\@y{\@s{0}%
 Simplified nonce
}%
\@xx{}%
\@x{\@s{104.13} timestamp \.{\mapsto} nextBlockId ]\@s{4.1}}%
\@y{\@s{0}%
 Simplified timestamp
}%
\@xx{}%
\@x{\@s{43.91} updatedChain \.{\defeq} [}%
 \@x{\@s{64.41} blocks \.{\mapsto} chainsByNode [ n ] . blocks \.{\cup} \{
 nextBlockId \} ,\,}%
 \@x{\@s{64.41} edges \.{\mapsto} chainsByNode [ n ] . edges \.{\cup} \{
 {\langle} tipsByNode [ n ] ,\, nextBlockId {\rangle} \}}%
\@x{\@s{43.91} ]}%
 \@x{\@s{43.91} newWork \.{\defeq} workByNode [ n ] [ tipsByNode [ n ] ] \.{+}
 WorkFor ( newBlock )}%
 \@x{\@s{43.91} updatedWork \.{\defeq} [ workByNode [ n ] {\EXCEPT} {\bang} [
 nextBlockId ] \.{=} newWork ]}%
\@x{\@s{43.91} prevTip \.{\defeq} tipsByNode [ n ]}%
\@y{\@s{0}%
 Previous tip before creating the new block
}%
\@xx{}%
\@x{\@s{27.51} \.{\IN}}%
\@x{\@s{27.51}}%
...
\@xx{}%
 \@x{\@s{27.51} \.{\land} blocksByNode \.{'} \.{=} [ blocksByNode {\EXCEPT}
 {\bang} [ n ] \.{=} @ \.{\cup} \{ newBlock \} ]}%
\@x{\@s{27.51}}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{\@s{27.51} \.{\land} chainsByNode \.{'} \.{=} [ chainsByNode {\EXCEPT}
 {\bang} [ n ] \.{=} updatedChain ]}%
\@x{\@s{27.51}}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{\@s{27.51} \.{\land} tipsByNode \.{'} \.{=} [ tipsByNode {\EXCEPT}
 {\bang} [ n ] \.{=} nextBlockId ]}%
\@x{\@s{27.51}}%
\@y{\@s{0}%
 Track total work for the new block at this node, even if it is a constant
 for now
}%
\@xx{}%
\@x{\@s{27.51} \.{\land} \.{\LET}}%
 \@x{\@s{46.82} newWorkMap \.{\defeq} [ b \.{\in} {\DOMAIN} workByNode [ n ]
 \.{\cup} \{ nextBlockId \} \.{\mapsto}}%
 \@x{\@s{63.22} {\IF} b \.{=} nextBlockId \.{\THEN} newWork \.{\ELSE}
 workByNode [ n ] [ b ] ]}%
\@x{\@s{46.82} \.{\IN}}%
 \@x{\@s{63.22} workByNode \.{'} \.{=} [ workByNode {\EXCEPT} {\bang} [ n ]
 \.{=} newWorkMap ]}%
\@x{\@s{27.51}}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{\@s{27.51} \.{\land} network \.{'} \.{=} [ m \.{\in} Nodes \.{\mapsto}
 {\IF} m \.{\neq} n \.{\THEN} network [ m ] \.{\cup} \{ newBlock \} \.{\ELSE}
 network [ m ] ]}%
\@x{\@s{27.51}}%
\@y{\@s{0}%
 Update the global \ensuremath{nextBlockId
}}%
\@xx{}%
\@x{\@s{27.51} \.{\land} nextBlockId \.{'} \.{=} nextBlockId \.{+} 1}%
\@x{\@s{27.51}}%
\@y{\@s{0}%
 Local node immediately confirms the previous tip
}%
\@xx{}%
 \@x{\@s{27.51} \.{\land} confirmedByNode \.{'} \.{=} [ confirmedByNode
 {\EXCEPT} {\bang} [ n ] \.{=} confirmedByNode [ n ] \.{\cup} \{ prevTip \}
 ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ ReceiveBlockWithPreviousKnown ( n ,\, block ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} block . prevHash \.{\in} chainsByNode [ n ] . blocks}%
\@x{\@s{16.4} \.{\land}}%
\@x{\@s{32.8} \.{\LET}}%
\@x{\@s{49.19} prevBlock \.{\defeq} block . prevHash}%
\@x{\@s{49.19} updatedChain \.{\defeq} [}%
 \@x{\@s{144.03} blocks \.{\mapsto} chainsByNode [ n ] . blocks \.{\cup} \{
 block . id \} ,\,}%
 \@x{\@s{144.03} edges \.{\mapsto} chainsByNode [ n ] . edges \.{\cup} \{
 {\langle} prevBlock ,\, block . id {\rangle} \}}%
\@x{\@s{116.94} ]}%
 \@x{\@s{49.19} newWork \.{\defeq} workByNode [ n ] [ tipsByNode [ n ] ] \.{+}
 WorkFor ( block )}%
 \@x{\@s{49.19} newTip \.{\defeq} {\IF} newWork \.{>} workByNode [ n ] [
 tipsByNode [ n ] ]}%
\@x{\@s{90.23} \.{\THEN} block . id}%
\@x{\@s{90.23} \.{\ELSE} tipsByNode [ n ]}%
\@x{\@s{32.8} \.{\IN}}%
 \@x{\@s{32.8} \.{\land} blocksByNode \.{'} \.{=} [ blocksByNode {\EXCEPT}
 {\bang} [ n ] \.{=} @ \.{\cup} \{ block \} ]}%
\@x{\@s{32.8}}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{\@s{32.8} \.{\land} network \.{'} \.{=} [ network {\EXCEPT} {\bang} [ n ]
 \.{=} @ \.{\,\backslash\,} \{ block \} ]}%
 \@x{\@s{32.8} \.{\land} chainsByNode \.{'} \.{=} [ chainsByNode {\EXCEPT}
 {\bang} [ n ] \.{=} updatedChain ]}%
\@x{\@s{32.8} \.{\land} \.{\LET}}%
 \@x{\@s{48.01} newWorkMap \.{\defeq} [ b \.{\in} {\DOMAIN} workByNode [ n ]
 \.{\cup} \{ block . id \} \.{\mapsto}}%
 \@x{\@s{64.41} {\IF} b \.{=} block . id \.{\THEN} newWork \.{\ELSE}
 workByNode [ n ] [ b ] ]}%
\@x{\@s{48.01} \.{\IN}}%
 \@x{\@s{64.41} workByNode \.{'} \.{=} [ workByNode {\EXCEPT} {\bang} [ n ]
 \.{=} newWorkMap ]}%
\@x{\@s{32.8}}%
\@y{\@s{0}%
...
\@xx{}%
 \@x{\@s{32.8} \.{\land} {\IF} newWork \.{>} workByNode [ n ] [ tipsByNode [ n
 ] ] \.{\THEN}}%
 \@x{\@s{64.26} \.{\land} confirmedByNode \.{'} \.{=} [ confirmedByNode
 {\EXCEPT} {\bang} [ n ] \.{=} confirmedByNode [ n ] \.{\cup} \{ tipsByNode [
 n ] \} ]}%
\@x{\@s{56.06} \.{\ELSE}}%
\@x{\@s{64.26} \.{\land} confirmedByNode \.{'} \.{=} confirmedByNode}%
\@x{\@s{32.8}}%
...
\@xx{}%
 \@x{\@s{32.8} \.{\land} tipsByNode \.{'} \.{=} [ tipsByNode {\EXCEPT} {\bang}
 [ n ] \.{=} block . id ]}%
\@x{\@s{32.8} \.{\land} {\UNCHANGED} {\langle} nextBlockId {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Next state relation
...
\@xx{}%
\@x{ Next \.{\defeq}}%
\@x{\@s{16.4} \.{\lor} \E\, n \.{\in} Nodes \.{:} CreateBlock ( n )}%
 \@x{\@s{16.4} \.{\lor} \E\, n \.{\in} Nodes \.{:} \E\, block \.{\in} network
 [ n ] \.{:} ReceiveBlockWithPreviousKnown ( n ,\, block )}%
\@pvspace{16.0pt}%
\@x{}%
\@y{\@s{0}%
 \ensuremath{vars \.{\defeq} {\langle}blocksByNode,\, chainsByNode,\,
 tipsByNode,\, workByNode,\, confirmedByNode,\, network,\,
 nextBlockId{\rangle}
}}%
\@xx{}%
\@pvspace{8.0pt}%
...
\@x{\@s{16.4} \.{\land} \A\, n \.{\in} Nodes \.{:}}%
\@x{\@s{31.61} \.{\land} nextBlockId \.{\in} Nat}%
\@x{\@s{31.61} \.{\land} tipsByNode [ n ] \.{\in} Nat}%
\@x{\@s{31.61} \.{\land} chainsByNode [ n ] . blocks \.{\subseteq} Nat}%
 \@x{\@s{31.61} \.{\land} chainsByNode [ n ] . edges \.{\subseteq} ( Nat
 \.{\times} Nat )}%
 \@x{\@s{31.61} \.{\land} \A\, blockId \.{\in} {\DOMAIN} workByNode [ n ]
 \.{:}}%
\@x{\@s{54.04} \.{\land} blockId \.{\in} Nat}%
\@x{\@s{54.04} \.{\land} workByNode [ n ] [ blockId ] \.{\in} Nat}%
\@x{\@s{31.61} \.{\land} confirmedByNode [ n ] \.{\subseteq} Nat}%
\@x{\@s{31.61} \.{\land} network [ n ] \.{\subseteq} Block}%
\@x{\@s{31.61} \.{\land} \A\, b \.{\in} blocksByNode [ n ] \.{:}}%
\@x{\@s{46.82} \.{\land} b . id \.{\in} Nat}%
\@x{\@s{46.82} \.{\land} b . prevHash \.{\in} Nat}%
\@x{\@s{46.82} \.{\land} b . nonce \.{\in} Nat}%
\@x{\@s{46.82} \.{\land} b . timestamp \.{\in} Nat}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Properties
...
\@xx{}%
\@x{ TipHasMostWork \.{\defeq}}%
\@x{\@s{16.4} \A\, n \.{\in} Nodes \.{:}}%
\@x{\@s{27.72} \A\, b \.{\in} chainsByNode [ n ] . blocks \.{:}}%
 \@x{\@s{39.04} workByNode [ n ] [ tipsByNode [ n ] ] \.{\geq} workByNode [ n
 ] [ b ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
